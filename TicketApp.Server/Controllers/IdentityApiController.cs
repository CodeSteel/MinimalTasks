...
public static class IdentityApiEndpointRouteBuilderExtensions
{
        ...
        where TUser : class, new()
    {
        ...
        routeGroup.MapPost("/register", async Task<Results<Ok, ValidationProblem>>
            ([FromBody] CustomRegisterRequest registration, HttpContext context, [FromServices] IServiceProvider sp, [FromServices] AppStatsService appStatsService) =>
        {
            ...
            var user = new TUser();
            await userStore.SetUserNameAsync(user, (registration.FirstName + "_" + registration.LastName), CancellationToken.None);
            ...
            await appStatsService.AddToStat(AppStatType.UsersRegistered, 1);
            ...
            ([FromBody] LoginRequest login, [FromQuery] bool? useCookies, [FromQuery] bool? useSessionCookies, [FromServices] IServiceProvider sp) =>
        {
            ...
            var dataContext = sp.GetRequiredService<ApplicationDataContext>();
            ...
            if (!result.Succeeded)
            {
                // try get username from email
                User? queriedUser = await dataContext.Users.FirstOrDefaultAsync(x => x.Email!.ToLower() == login.Email.ToLower());
                if (queriedUser != null)
                {
                    result = await signInManager.PasswordSignInAsync(queriedUser.UserName!.ToLower(), login.Password, isPersistent, lockoutOnFailure: true);
